    ...
    paypal.Buttons({
        createOrder: function(data, actions) {
            return actions.order.create({
                purchase_units: [{
                    description: 'PDF-Dokument: Widerspruch Mehrbedarf', // Angepasste Beschreibung
                    amount: {
                        value: '0.99',
                        currency_code: 'EUR'
                    }
        ...
        onApprove: function(data, actions) {
            console.log("Zahlung erfolgreich! Bereite Mehrbedarf-Dokument vor...");

            // Wir führen die Validierung für dieses Formular durch
            const formData = getFormData();
            if (formData.widerspruchsgruende.length === 0 && document.getElementById("ergaenzendeArgumenteMehrbedarf").value.trim() === "" ) {
                alert("Bitte wählen Sie mindestens einen Mehrbedarf aus, bevor Sie die Erstellung abschließen.");
                return;
            }
             if (document.getElementById("forderungMehrbedarf").value.trim() === "") {
