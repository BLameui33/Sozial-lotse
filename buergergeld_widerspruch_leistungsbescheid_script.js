    ...
    paypal.Buttons({
        createOrder: function(data, actions) {
            return actions.order.create({
                purchase_units: [{
                    description: 'PDF-Dokument: Widerspruch Bürgergeld-Bescheid',
                    amount: {
                        value: '0.99',
                        currency_code: 'EUR'
                    }
        ...
        onApprove: function(data, actions) {
            console.log("Zahlung erfolgreich! Bereite Bürgergeld-Widerspruch vor...");

            // Wir führen die Validierung für dieses Formular durch
            const formData = getFormData();
            if (!formData.hauptgrundWiderspruch) {
                alert("Bitte wählen Sie einen Hauptgrund für Ihren Widerspruch.");
                return;
            }
            if (document.getElementById("antragImWiderspruch").value.trim() === "") {
                 alert("Bitte geben Sie an, was Sie mit dem Widerspruch erreichen möchten.");
                 return;
            }
