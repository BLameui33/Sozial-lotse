// ====================================================================================
// HINWEIS: Die globalen Hilfsfunktionen und die generateAlg1AblehnungWiderspruchPDF()-
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const form = document.getElementById('widerspruchAlg1AblehnungForm');
    const saveBtn = document.getElementById('saveBtnAlg1Ablehnung');
    const loadBtn = document.getElementById('loadBtnAlg1Ablehnung');
    const storageKey = 'widerspruchAlg1AblehnungFormData_v1';

    // --- Steuerung der dynamischen Detail-Felder (bleibt unverändert) ---
    const widerspruchsgrundCheckboxes = document.querySelectorAll('input[name="widerspruchsgrundAlg1Ablehnung"]');
    ...
    // --- WICHTIG: Ihre getFormData() Funktion für ALG1 bleibt exakt so wie sie ist! ---
    function getFormData() {
        ...
        document.querySelectorAll('input[name="widerspruchsgrundAlg1Ablehnung"]:checked').forEach(cb => data.widerspruchsgruende.push(cb.value));
        return data;
    }

    // --- Ihre Speichern & Laden Logik bleibt unverändert ---
    function populateForm(data) {
        const formElementIds = ["personName", "kundenNummer", "anzahlPersonenAlg1Ablehnung", "personAdresse", "personPlz", "personOrt", "agenturName", "agenturAdresse", "bescheidDatum", "bescheidAktenzeichen", "ablehnungsgrundVermieter", "textAnwartschaft", "textArbeitslosigkeit", "textVerfuegbarkeit", "textArbeitslosmeldung", "ergaenzendeArgumenteAlg1Ablehnung", "forderungAlg1Ablehnung"];
        formElementIds.forEach(id => { const element = document.getElementById(id); if (element && data[id] !== undefined) { element.value = data[id]; } });
        document.querySelectorAll('input[name="widerspruchsgrundAlg1Ablehnung"]').forEach(cb => { if (cb) { cb.checked = !!(data.widerspruchsgruende && data.widerspruchsgruende.includes(cb.value)); toggleDetailDiv(cb); } });
    }
    if (saveBtn) {
    ...
    paypal.Buttons({
        createOrder: function(data, actions) {
            return actions.order.create({
                purchase_units: [{
                    description: 'PDF-Dokument: Widerspruch Ablehnung ALG 1', // Angepasste Beschreibung
                    amount: {
                        value: '0.99',
                        currency_code: 'EUR'
                    }
        ...
        onApprove: function(data, actions) {
            console.log("Zahlung erfolgreich! Bereite ALG1-Dokument vor...");

            // Wir führen die Validierung für dieses Formular durch
            const formData = getFormData();
            if (formData.widerspruchsgruende.length === 0 && document.getElementById("ergaenzendeArgumenteAlg1Ablehnung").value.trim() === "" ) {
                alert("Bitte wählen Sie mindestens einen Widerspruchsgrund aus, bevor Sie die Erstellung abschließen.");
                return;
            }
             if (document.getElementById("forderungAlg1Ablehnung").value.trim() === "") {
            ...
            localStorage.setItem('pendingPaymentData-alg1', JSON.stringify(formData));

            // Zur Danke-Seite weiterleiten (mit neuem Typ!)
            window.location.href = "danke.html?typ=alg1";
        },
